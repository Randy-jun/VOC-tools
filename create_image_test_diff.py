...
for dataset in datasets:
    new_image_path = os.path.join(root_dir, dataset , "percent_image")
    mew_label_path = os.path.join(root_dir, dataset , "percent_label")
    ...
    for k, label_path in enumerate(label_paths):
        ...
        tmp_size = img.shape #HWC
        img_copy = cv2.resize(img, (INPUT_IMAGE_SIZE, INPUT_IMAGE_SIZE), interpolation = cv2.INTER_AREA)
        ...
        for k, value in enumerate(x_label):
            ...
            if (value[0] == 'name'):
                ...
                tmp_xmin = int((x_label[k + 1][0] * INPUT_IMAGE_SIZE) // tmp_size[1])
                # print(tmp_xmin)
                tmp_xmax = int((x_label[k + 1][1] * INPUT_IMAGE_SIZE) // tmp_size[1])
                # print(tmp_xmax)
                tmp_ymin = int((x_label[k + 2][0] * INPUT_IMAGE_SIZE) // tmp_size[0])
                # print(tmp_ymin)
                tmp_ymax = int((x_label[k + 2][1] * INPUT_IMAGE_SIZE) // tmp_size[0])
                # print(tmp_ymax)
                
                sub_img = img_copy[tmp_ymin:tmp_ymax, tmp_xmin:tmp_xmax]
                ...
                tmp_file = os.path.join(new_image_path, x_label[0][1][:-4] + win_name + ".jpg")
                # print(tmp_file)
    ...
    with open(os.path.join(mew_label_path, "label.txt"), 'w') as fo:
