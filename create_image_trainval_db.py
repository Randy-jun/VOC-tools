...
def main():
	...
	db_path = os.path.expanduser("~/data/VOCdevkit/dataDB/trainvlaDB_t100_lmdb")
	...
	with open(label_paths, 'r') as fi:
		...
		for lof in lsof:
			...
			labels.append(lof.split()[1][:-1].find("1"))
		fi.close()

	# print(image_paths[0:3])
	# print(labels[0:3])
	create_data_db(db_path, image_paths[:100], labels[:100])
