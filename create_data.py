...
def create_annoset(root_dir, list_file, out_dir, example_dir, redo = False, anno_type = "detection",\
        label_type = "xml", backend = "lmdb", check_size = False, encode_type = "", encoded = False,\
        gray = False, label_map_file = "", min_dim = 0, max_dim = 0, resize_height = 0, resize_width = 0,\
        shuffle = False, check_label = False):
...
data_root_dir = os.environ["HOME"] + "/data/VOCdevkit"
script_dir = os.getcwd()
redo = True
dataset_name = "VOC0712"
map_file = os.path.join(script_dir, "labelmap_voc.prototxt")
anno_type = "detection"
label_type = "xml"
db_type = "lmdb"
check_size = False
...
for dataset in datasets:
    list_file = os.path.join(script_dir, dataset + ".txt")
    create_annoset(data_root_dir, list_file, script_dir, script_dir, redo, anno_type, \
        label_type, db_type, check_size, encode_type, encoded, gray, map_file, min_dim, max_dim, \
        height, width, shuffle, check_label)
